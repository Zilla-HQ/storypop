...
> **What this is:** the design document submitted to Google as part of the **Google Ads API Token Application** for Basic Access (the form at https://ads.google.com/aw/apicenter once you have an MCC).
>
> **Filing cadence:** ONCE per Zilla HQ org, not per merchant. The MCC `3797064633` (Zilla HQ) holds the developer token; all future portfolio merchants reuse it.
>
> **When to amend:** if Zilla HQ ever wants to expand scope — e.g., adding YouTube preroll campaigns, Customer Match audiences, or migrating to Smart Bidding — Google may require an updated design doc filing. Edit this template, regenerate the PDF (Safari → Save as PDF), and submit via the API Center "Update tool details" flow.
>
> **PDF generation:** see `GOOGLE_ADS.md` §2.2. Quick path on macOS: open the rendered HTML in Safari (`npm install marked && node -e "const{marked}=require('marked');const fs=require('fs');console.log(marked.parse(fs.readFileSync('docs/google-ads-design-doc-template.md','utf8')))" > /tmp/x.html && open -a Safari /tmp/x.html`), then File → Export as PDF.
>
> **Last submitted:** 2026-05-06, Basic Access, by Zilla HQ (operator: jack@seifdn.org). Approval pending.
